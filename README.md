...
Download them from the repo **[Releases](https://github.com/tuseeq-toor/TV-Game-Controller/releases)** page (`TV_Gamepad_phone.apk` and `TV_Gamepad_Host_tv.apk`). Version tags (`v*`) create a normal release. Every pull request also publishes a prerelease named **PR #N APKs**. Or build them on a computer:
